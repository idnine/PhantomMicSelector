...
int main(void) {
	...
	BCSCTL1 = CALBC1_1MHZ;			// Internal Clock Set 1MHz
	DCOCTL = CALDCO_1MHZ;
...
__interrupt void selButton(void) {
	P1IE &= ~(SW1 + SW2 + SW3 + SW4);		// Switch Interrupt Disable
	if(P1IFG & SW1) {
		selectCh = 1;
	} else if(P1IFG & SW2) {
		selectCh = 2;
	...
	if(currCh == selectCh) {
		if(isMute) {
			// goes UN-MUTE
			isMute = 0;
			TACCTL0 &= ~CCIE;		// LED Blink, Timer Off
			P1OUT &= ~EN;
			P2OUT &= ~(0x02 << currCh);	// Current Ch. LED ON
		} else {
			// goes MUTE
			isMute = 1;
			P1OUT |= EN;
			TACCTL0 |= CCIE;		// LED Blink, Timer ON
		}
	} else {
		P2OUT |= (0x02 << currCh);
		currCh = selectCh;
		TACCTL0 &= ~CCIE;
		isChange = 1;
		P1OUT |= EN;				// Channel MUTE during port change
